...
class Atlasv3(DRHMC_AdaptiveStepsize):
    ...
    def __init__(self, D, log_prob, grad_log_prob, mass_matrix=None, 
        ...
        super(Atlasv3, self).__init__(D=D, log_prob=log_prob, grad_log_prob=grad_log_prob, 
    ...
    def preliminary_step(self, q, p, step_size, offset, n_leapfrog_input=None):
        ...
        # if (Nuturn < self.min_nleapfrog) or (not success) :
        if (Nuturn <= self.min_nleapfrog)  :
            ...
            if lp_N == -np.inf:
                ...
                return [q, p], [qlist, plist, glist], log_prob_list, Hs, 0
    ...
    def delayed_step(self, q0, p0, qlist, glist, step_size, offset, n_leapfrog, log_prob_accept_first, log_prob_H_first):
        ##
        try:
            ...
            if self.constant_trajectory == 2:
                ...
                n_leapfrog_new = int(min(self.max_nleapfrog, n_leapfrog*(step_size / step_size_new)**0.5))
            else:
    ...
    def delayed_step_upon_failure(self, q0, p0, step_size): 
        ...
        try:
            ...
            # Ghost trajectory for the second proposal
            try:
                Nuturn_ghost, _, _, _, success_ghost = self.nuts_criterion(q1, -p1, step_size)            
                    ...
                    if self.probabilistic == 1:
                        if log_prob_N == -np.inf:
                            ...
                            return self.delayed_step(q, p, qlist, glist,
                                                    ...
                                                    log_prob_accept_first=log_prob_accept, log_prob_H_first=log_prob_H)
...
#     def preliminary_step(self, q, p, step_size, offset):
#         """
#         First step is making a no-uturn proposal.
#         """
#         try:
...
#         if not success:
#             log_prob_list = [-np.inf, -np.inf, -np.inf] # log prob, log prob H, log prob N
#             qpg_list = [qlist, plist, glist]
#             Hs, n_leapfrog = [0, 0], 0
#             return [q, p], qpg_list, log_prob_list, Hs, success
        
#         else:
#             n_leapfrog, lp_N = self.nleapfrog_sample_and_lp(Nuturn,  offset)
#             q1, p1 = qlist[n_leapfrog], plist[n_leapfrog]           
#             # Go backward
#             Nuturn_rev = self.nuts_criterion(q1, -p1, step_size)[0]
#             n_leapfrog_rev, lp_N_rev = self.nleapfrog_sample_and_lp(Nuturn_rev, offset, nleapfrog=n_leapfrog)
...
#             return [q1, p1], [qlist, plist, glist], log_prob_list, Hs, success
...
#     def delayed_step_upon_failure(self, q0, p0, step_size, qpg_list): 
#         """Delayed step upon failure is executed when Nuturn of first step < min_nleapfrog.
#         """
#         try:
#             # Estimate the Hessian and the stepsize given the rejected trajectory
...
#             # Ghost trajectory for the second proposal
#             try:
#                 Nuturn_ghost, qlist_ghost, plist_ghost, glist_ghost, success_ghost = self.nuts_criterion(q1, -p1, step_size)
#                 if success_ghost :
#                     qf, pf, accepted = q0, p0, -24
#                     return qf, pf, accepted, Hs               
#             except Exception as e:
...
#         p =  multivariate_normal.rvs(mean=np.zeros(self.D), cov=self.inv_mass_matrix, size=1)
#         offset = self.offset_function()
#         qp, qpg_list, log_prob_list, Hs, success = self.preliminary_step(q, p, step_size, offset=offset)
#         log_prob_accept, log_prob_H, log_prob_N = log_prob_list
#         q1, p1 = qp
#         qlist, plist, glist = qpg_list
...
#         # Hastings
#         if not success :
#             return self.delayed_step_upon_failure(q, p, step_size=step_size, qpg_list=qpg_list)
#         else:
#             u =  np.random.uniform(0., 1., size=1)
#             if np.isnan(log_prob_accept): log_prob_accept = -np.inf
#             if  np.log(u) > min(0., log_prob_accept): #reject
