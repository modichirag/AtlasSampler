    ...
    def preliminary_step(self, q, p, step_size, offset, n_leapfrog=None):
        ...
        try:
            ...
            if Nuturn < self.min_nleapfrog :
                ...
                n_leapfrog_input = n_leapfrog
                if (n_leapfrog is not None):
                    
                    # if (n_leapfrog == 0) & (Nuturn > 0): #Occurs only in delayed-failure & seems like ghost is running.
                    #     log_prob_list, Hs = [0., 0., 0.], [0., 0.]
                    #     return [q, p], [qlist, plist, glist], log_prob_list, Hs, Nuturn                                                
                ...
                #    print(n_leapfrog_input, n_leapfrog, Nuturn, offset, lp_N, lp_N_rev)
                log_prob_accept = min(0, log_prob_accept) #THIS HAS A PROBLEM WHEN log_prob is NAN
                log_prob_list = [log_prob_accept, log_prob_H, log_prob_N]

                # if (n_leapfrog_input is not None):
                #     if (n_leapfrog_input == 0) & (Nuturn > 0): #Occurs only in delayed-failure & seems like ghost is running.
                #         log_prob_list, Hs = [0., 0., 0.], [0., 0.]
                #         return [q, p], [qlist, plist, glist], log_prob_list, Hs, Nuturn                        
    ...
    def delayed_step_upon_failure(self, q0, p0, qlist, glist, step_size, offset, n_leapfrog, log_prob_accept_first):
        ...
        try:
            ...
            if n_leapfrog_ghost != 0: #in this case, ghost stage cannot lead to the current delayed propsal
                ...
                # Hastings
                log_prob_delayed = np.log((1-np.exp(log_prob_accept_ghost))) - np.log((1- np.exp(log_prob_accept_first)))
                #if log_prob_delayed  != 0:
                #    print(log_prob_delayed, log_prob_accept_ghost, log_prob_accept_first, n_leapfrog, n_leapfrog_ghost, len(qlist_ghost), log_prob_ghost_list)
                log_prob_eps = epsf2.logpdf(step_size_new) - epsf1.logpdf(step_size_new)
                log_prob_accept = log_prob_H + log_prob_delayed + log_prob_eps
                ...
                if np.isnan(log_prob_accept) or (q0-q1).sum()==0:
                    ...
                    if len(qlist_ghost) > self.min_nleapfrog : print("WRONG ACCEPTANCE",  n_leapfrog_ghost, len(qlist_ghost), log_prob_ghost_list)                    
                    if len(qlist_ghost) > 2 : print("WRONG ACCEPTANCE  2", n_leapfrog_ghost, len(qlist_ghost), log_prob_ghost_list)
