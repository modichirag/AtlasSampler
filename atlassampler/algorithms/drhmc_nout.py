...
class DRHMC(HMC):
    ...
    def __init__(self, log_prob, grad_log_prob=None, log_prob_and_grad=None, invmetric_diag=None):
        super(DRHMC, self).__init__(log_prob, grad_log_prob, log_prob_and_grad)       
    ...
    def get_num(self, mj, q, p, nleap, step_size, stepfactor, H0): 
        ...
        for j in range(mj):
            fac = stepfactor**(j)
            qj, pj, _, _ = self.leapfrog(q, p, int(nleap*fac), step_size/fac)
            Hj = self.H(qj, pj)
            pfac = np.exp(H0 - Hj)
            if  (q - qj).sum()==0: 
                pfac = 0.
    ...
    def step(self, q, nleap, step_size, nprops, stepfactor):
        ...
        for j in range(nprops):
            ...
            qj, pj, _, _ = self.leapfrog(q0, p0, nleapf, step_sizef)            
            Hj = self.H(qj, pj)
            pfac = np.exp(H0 - Hj)
            if  (q0 - qj).sum()==0:
                pfac = 0.
    ...
    def _parse_kwargs_sample(self, **kwargs):
        ...
        self.nsamples = kwargs['n_samples']
        self.burnin = kwargs['n_burnin']
        self.step_size = kwargs['step_size']
        self.nleap = kwargs['nleap']

    def sample(self, q, p=None, callback=None, seed=0, **kwargs):
        kw = kwargs
...
class DRHMC_NoUT(DRHMC_AdaptiveStepsize):
    ...
    def __init__(self, D, log_prob, grad_log_prob, mass_matrix=None,
        ...
        super(DRHMC_NoUT, self).__init__(D=D, log_prob=log_prob, grad_log_prob=grad_log_prob, 
    ...
    def delayed_step(self, q0, p0, qlist, glist, step_size, offset, n_leapfrog, log_prob_list_first): 
        ...
        try:
            ...
            step_size_new = step_size / self.step_factor
            ...
            if n_leapfrog_ghost == 0: #in this case, ghost stage cannot lead to the current delayed propsal
                ...
                if (self.probabilistic >= 1) & (log_prob_N_ghost == -np.inf): 
                    ...
                    log_prob_eps = 0. #epsf2.logpdf(step_size_new) - epsf1.logpdf(step_size_new)
