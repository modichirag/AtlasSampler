...
def cumulative_rmse(samples, counts, true_val=None, true_scatter=None, ref_samples=None, mode=1, nevals=None, relative=False, verbose=False):
...
def cumulative_rmse_per_chain(samples, counts, true_val=None, ref_samples=None, mode=1, nevals=None, relative=False, verbose=False):
    ...
    count_list, err_list = [], []
    for i in range(nchains):
        count, err = cumulative_rmse(samples[i:i+1], counts[i:i+1], true_val=true_val, ref_samples=ref_samples, mode=mode, nevals=nevals, relative=relative, verbose=verbose)
        count_list.append(count)
        err_list.append(err)
